...
namespace theoretica {
	...
	public:
		...
		inline polynomial operator/(const polynomial& d) const {
			...
			// Check if division failed to converge
			if(i > this_order && r.degree() >= d_order) {
		...
		inline bool operator==(const polynomial& other) const {
			...
			for (size_t i = 0; i <= this_degree; ++i)
				if (abs(coeff[i] - other[i]) > MACH_EPSILON)
					return false;
		...
		inline vec<complex<>, 2> quadratic_roots() const {
			...
			complex<> z1, z2;
			const Type a = coeff[2];
			const Type b = coeff[1];
			const Type c = coeff[0];
			...
			const Type discriminant = b * b - 4 * a * c;
			const complex<> delta = sqrt(complex<>(discriminant));

			const real sign = (b > 0) ? 1 : -1;
			const complex<> q = -0.5 * (b + sign * delta);
		...
		/// Uses numerically stable pairwise multiplication with sorted roots
		template<typename Vector, enable_vector<Vector> = true>
		inline static polynomial<Type> from_roots(const Vector& roots) {

			polynomial<Type> p;
			p.resize(roots.size() + 1);
			p[0] = Type(1.0);

			for (size_t i = 0; i < roots.size(); i++)
				p *= polynomial<Type>({-roots[i], 1});
		...
		inline static polynomial<Type> monomial(Type c, unsigned int order) {
			...
			polynomial m;
			m.coeff = vec<Type>(order + 1, Type(0.0));
			m.coeff[order] = c;
			return m;
		}
