...
namespace uroboro {
	...
	dual2 sqrt(dual2 x) {
		...
		if(sqrt_x == 0) {
			UMATH_ERROR("sqrt(dual2)", sqrt_x, DIV_BY_ZERO);
	...
	dual2 tan(dual2 x) {
		...
		if(cos_x == 0) {
			UMATH_ERROR("tan(dual2)", cos_x, DIV_BY_ZERO);
			return dual2(nan(), nan());
		}
	...
	dual2 cot(dual2 x) {
		...
		if(sin_x == 0) {
			UMATH_ERROR("cot(dual2)", sin_x, DIV_BY_ZERO);
			return dual2(nan(), nan());
		}
	...
	dual2 ln(dual2 x) {
		...
		if(x.Re() <= 0) {
			UMATH_ERROR("ln(dual2)", x.Re(), OUT_OF_DOMAIN);
			return dual2(nan(), nan());
		}
	...
	dual2 log2(dual2 x) {
		...
		if(x.Re() <= 0) {
			UMATH_ERROR("log2(dual2)", x.Re(), OUT_OF_DOMAIN);
			return dual2(nan(), nan());
		}
	...
	dual2 log10(dual2 x) {
		...
		if(x.Re() <= 0) {
			UMATH_ERROR("log10(dual2)", x.Re(), OUT_OF_DOMAIN);
			return dual2(nan(), nan());
		}
	...
	dual2 asin(dual2 x) {
		...
		if(x.Re() >= 1) {
			UMATH_ERROR("asin(dual2)", x.Re(), OUT_OF_DOMAIN);
			return dual2(nan(), nan());
		}
	...
	dual2 acos(dual2 x) {
		...
		if(x.Re() >= 1) {
			UMATH_ERROR("acos(dual2)", x.Re(), OUT_OF_DOMAIN);
			return dual2(nan(), nan());
		}
