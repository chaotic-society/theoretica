...
// Macro to pick the right overload of functions
#define BENCHMARK_REAL(func, opt) \
	ctx.benchmark(#func, CAST_LAMBDA(func, real), opt)


int main(int argc, char const *argv[]) {
	
	auto ctx = benchmark::make_context("real_analysis", argc, argv);
	ctx.settings.outputFiles = { "test/benchmark/benchmark_real_analysis.csv" };
	...
	auto pow_opt = benchmark::benchmark_options<real>(
		10, 100'000,
		benchmark::generator::uniform1D(-100, +100)
	);
